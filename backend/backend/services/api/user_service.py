...
from backend.services.auth_service import create_access_token, hash_password, decode_access_token 
...
class UserService(CrudBaseService):
    ...
    async def get_user_by(self, /, email: str=None, user_id: int=None) -> model.User | None:
        ...
        if email:
            ...
            self._ra
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Argument missing")
    ...
    async def confirm_email(self, token: str) -> None:
        ...
        if user.email_verified:
            raise HTTPException(status_code=404, detail="Email has already been verified!")
