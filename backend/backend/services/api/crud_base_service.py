...
class CrudBaseService(CrudUtilMixin[T], Generic[T]):
    ...
    async def get_by_id(self, id: int, mask_class: BaseModel = None) -> T | None:
        ...
        if mask_class:
            stmt = sa.select(self.sa_type_).where(getattr(self.sa_type_, self.pk_name) == id)
    ...
    async def update(self, update_model: BaseModel, /, id: int=None, entity: T=None, patch: bool=True):
